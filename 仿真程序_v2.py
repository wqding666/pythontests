...
def angular_spectrum_propagator(u_in: np.ndarray, 
                              ...
                              dy: float) -> np.ndarray:
    """
    角谱法自由空间传播算子（支持四维输入张量 BxCxHxW）
    实现从任意前置维度的输入到输出平面的批量传播，保持原有物理模型不变
    
    Args:
        u_in: 输入复振幅场，支持多维形状如 (..., H, W)
        z: 传播距离 (m)
        wavelength: 波长 (m)
        dx, dy: 空间采样间隔 (m)
    
    Returns:
        u_out: 输出复振幅场，形状与输入一致
    """
    # 输入验证
    if not np.iscomplexobj(u_in):
        warnings.warn("Input is not complex-valued; converting to complex.", UserWarning)
        u_in = u_in.astype(np.complex128)
        
    if u_in.ndim < 2:
        raise ValueError("Input must have at least two spatial dimensions (H, W).")
    
    # 提取空间维度并保留原始形状
    H, W = u_in.shape[-2:]
    original_shape = u_in.shape
    leading_dims = original_shape[:-2]
    batch_size_total = np.prod(leading_dims) if leading_dims else 1
    
    # 重塑为二维批处理格式
    u_in_reshaped = u_in.reshape((int(batch_size_total), H, W))
    ...
    H_prop = np.exp(1j * kz * z)  # (H, W)
    
    # 批量FFT变换与传播（利用广播机制）
    U_fft_batch = fft2(u_in_reshaped, axes=(-2, -1))  # 对最后两个轴进行变换
    u_out_reshaped = ifft2(U_fft_batch * H_prop[np.newaxis, :, :], axes=(-2, -1))
    
    # 恢复原始多维结构
    u_out = u_out_reshaped.reshape(original_shape)
...
def angular_spectrum_propagator_pytorch(u_in: torch.Tensor, 
                                     ...
                                     device: str = 'cpu') -> torch.Tensor:
    """
    PyTorch版本角谱传播（在CPU上执行NumPy操作，支持四维张量）
    自动处理BxCxHxW输入，并返回相同形状的Tensor
    """
